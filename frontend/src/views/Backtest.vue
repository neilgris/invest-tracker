...
const PARAM_DESC = {
  stop_loss_pct: '兜底止损（从入场价算）：浮亏达此%强制卖出，所有退出模式均生效。\n'
    + '· simple/profit_retention/cost_protection：主要保护线，日常可触发\n'
    + '· pmax_drawdown：绝对底线，建议设在 pmax 值的 1.5-2× 以上，否则 pmax 形同虚设\n'
    + '· ma_cross：仅在价格跳空越过均线时生效，建议宽松设置（15-25%）',
  max_hold_days:        '时间止损：持仓超过此天数无论盈亏强制平仓，防止资金长期被套。0=不限制。常设365天（1年）或730天（2年）。',
  take_profit_pct:      '止盈线（从入场价算）：浮盈涨到此%时卖出锁利。过低会频繁止盈、错失后续上涨；建议参考标的历史平均涨幅周期设定。',
  pmax_drawdown_pct:    '移动止损容忍度（从持仓期最高价算）：价格从历史高点回落超过此%触发出场。\n'
    + '⚠️ 与 stop_loss_pct 基准不同：stop_loss 从入场价算，pmax 从持仓最高价算。\n'
    + '合理配置：pmax_drawdown_pct < stop_loss_pct，否则兜底止损会先于 pmax 触发，pmax 失效。\n'
    + '示例：止损10%、pmax15%——价格涨到120%再跌15%=102%，此时从入场价仍有+2%，止损不触发，pmax正常工作。',
  profit_trigger_pct:   '激活阈值（浮盈保留/成本保护）：浮盈须先达到此%，保护机制才生效。激活前只有兜底止损。建议设在你认为"已有可观盈利"的水平（如15-30%）。',
  profit_retention_pct: '浮盈保留比（0～1）：激活后，止损线 = 历史最高浮盈% × 此比例。\n'
    + '示例：最高浮盈30%，保留比0.6 → 止损线=+18%，跌到18%以下就出场。\n'
    + '边界：0=保留0%浮盈（跌回入场价即出，等同保本）；1=保留全部浮盈（几乎不触发）。\n'
    + '建议范围：0.3-0.8，过低接近保本保护，过高很难出场。',
  cost_trigger_pct:     '激活阈值（成本保护）：浮盈须先达到此%后保本保护才生效。建议至少5-10%，给策略一定获利空间再锁仓位。',
  cost_floor_pct:       '保护底线：激活后止损线 = 成本价×(1+此值/100)。0=保本，2=成本+2%，负值=允许小亏才触发（通常不推荐）。',
